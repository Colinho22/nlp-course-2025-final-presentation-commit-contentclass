...
\textbf{Discovery:} Two neurons working together can model the \textit{rise and fall} of temperature throughout the day!
...
\begin{exercise}[Hand Calculation with 3 Neurons]
\textbf{The 3-neuron network:}
\begin{itemize}
\item Baseline: $y_0 = 10$
\item Neuron 1: $y_1 = 15 \times \sigma(1.5 \times (x - 8))$ (handles left rise)
\item Neuron 2: $y_2 = 10 \times \sigma(2.0 \times (x - 12))$ (boosts peak)
\item Neuron 3: $y_3 = -15 \times \sigma(1.5 \times (x - 16))$ (handles right fall)
\end{itemize}

\textbf{Total output:} $y = y_0 + y_1 + y_2 + y_3$

\vspace{2mm}
\textbf{Calculate at the peak (x=12):}

\vspace{1mm}
\begin{enumerate}[label=\alph*)]
\item Neuron 1: $1.5 \times (12-8) = 6 \rightarrow \sigma(6) \approx 1.0$, so $y_1 = 15 \times 1.0 =$ \rule{2cm}{0.4pt}
\item Neuron 2: $2.0 \times (12-12) = 0 \rightarrow \sigma(0) =$ \rule{1cm}{0.4pt}, so $y_2 = 10 \times$ \rule{1cm}{0.4pt} $=$ \rule{2cm}{0.4pt}
\item Neuron 3: $1.5 \times (12-16) = -6 \rightarrow \sigma(-6) \approx 0.0$, so $y_3 = -15 \times 0.0 =$ \rule{2cm}{0.4pt}
\item \textbf{Total:} $y = 10 +$ \rule{1cm}{0.4pt} $+$ \rule{1cm}{0.4pt} $+$ \rule{1cm}{0.4pt} $=$ \rule{2cm}{0.4pt}
\end{enumerate}

\vspace{2mm}
\textbf{Compare to target:} $y = -(12-12)^2/10 + 20 = 20$ \\
How close is your answer? \rule{3cm}{0.4pt}
\end{exercise}
...
\textbf{Pattern:} More complex functions need more neurons, but each neuron handles a \textit{specific part} of the curve!
...
\item \textbf{Sigmoids are building blocks}: One sigmoid = S-curve, Multiple sigmoids = any shape
