...
\subtitle{BSc Enhanced Version with Full Formula Explanations}
...
% CHECKPOINT 1: Test Problem Understanding
\begin{frame}[t]{Checkpoint 1: Do You Understand the Problem?}
\begin{center}
\textbf{Quick Self-Test Before Moving Forward}
\end{center}
\vspace{3mm}
...
\textbf{Question 1:} Why can't N-grams solve the Paris problem?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 2:} What are the three memory mechanisms we need?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 3:} How far back do we need to remember?
...
We need: \textbf{Forget} (remove old info), \textbf{Input} (add new info), \textbf{Output} (reveal info when needed). Just like human selective memory!
}}
...
Real sentences can have important information 50-100 words back. LSTMs can handle this, but N-grams (1-2) and RNNs (5-10) both fail.
}}

\vspace{3mm}
\textbf{Key Insight:}

If you got these right, you understand WHY we need LSTMs! The next pages show HOW they work.
...
% NEW PAGE: Visual Math Breakdown
\begin{frame}[t]{Understanding the Math: Visual Explanations}
\begin{center}
\textbf{What Do These Symbols Actually DO?}
\end{center}
\vspace{2mm}

\begin{columns}[t]
\column{0.48\textwidth}
\textbf{1. Sigmoid Function $\sigma$:}

$$\sigma(z) = \frac{1}{1 + e^{-z}}$$

\vspace{2mm}
\plainenglish{Takes ANY number and squashes it to between 0 and 1. Used for gates because 0 = fully close, 1 = fully open}

\vspace{2mm}
\textbf{Visual Examples:}
\begin{itemize}
\item Input: $z = -5$ → $\sigma(-5) = 0.007$ ≈ 0 (CLOSE gate)
\item Input: $z = 0$ → $\sigma(0) = 0.5$ (HALF open)
\item Input: $z = +5$ → $\sigma(+5) = 0.993$ ≈ 1 (OPEN gate)
\end{itemize}

\vspace{3mm}
\textbf{2. Tanh Function:}

$$\tanh(z) = \frac{e^z - e^{-z}}{e^z + e^{-z}}$$

\vspace{2mm}
\plainenglish{Like sigmoid but outputs -1 to +1. Used for cell state because memory can be positive or negative}

\vspace{2mm}
\textbf{Visual Examples:}
\begin{itemize}
\item Input: $z = -3$ → $\tanh(-3) = -0.995$ ≈ -1
\item Input: $z = 0$ → $\tanh(0) = 0$
\item Input: $z = +3$ → $\tanh(+3) = +0.995$ ≈ +1
\end{itemize}

\column{0.48\textwidth}
\textbf{3. Element-wise Multiplication $\odot$:}

\vspace{2mm}
\plainenglish{Multiply each number separately, not matrix multiplication!}

\vspace{2mm}
\textbf{Example with actual numbers:}

\colorbox{backGray}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Gate values:} $f_t = [0.9, 0.5, 0.1]$\\
\textbf{Old memory:} $C_{t-1} = [0.8, 0.6, 0.4]$\\
\vspace{1mm}
\textbf{Result:} $f_t \odot C_{t-1} = [0.9 \times 0.8, 0.5 \times 0.6, 0.1 \times 0.4]$\\
\textbf{= [0.72, 0.30, 0.04]}\\
\vspace{1mm}
Each position multiplied independently!
}}

\vspace{3mm}
\textbf{4. Concatenation $[a, b]$:}

\vspace{2mm}
\plainenglish{Stick two vectors together end-to-end}

\vspace{2mm}
\textbf{Example:}

\colorbox{backGray}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Previous state:} $h_{t-1} = [0.5, 0.3]$ (size 2)\\
\textbf{Current word:} $x_t = [0.7]$ (size 1)\\
\vspace{1mm}
\textbf{Concatenated:} $[h_{t-1}, x_t] = [0.5, 0.3, 0.7]$ (size 3)\\
\vspace{1mm}
Just glued together!
}}
...
These operations are the BUILDING BLOCKS of every gate equation. Once you understand these 4 operations, the LSTM equations become much easier!
...
% CHECKPOINT 2: Test Cell State Understanding
\begin{frame}[t]{Checkpoint 2: Do You Understand Cell State?}
\begin{center}
\textbf{Test Your Understanding of the Memory Highway}
\end{center}
\vspace{3mm}
...
\column{0.48\textwidth}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 1:} Why is addition better than multiplication for memory?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 2:} What does the cell state equation $C_t = f_t \odot C_{t-1} + i_t \odot \tilde{C}_t$ do?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 3:} After 50 steps with $f_t = 0.95$, how much signal remains?
...
\vspace{3mm}
\colorbox{successGreen!10}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Answer 3: B (0.08)}

$0.95^{50} = 0.077$ ≈ 8\%. Still usable! Compare to RNN: $0.5^{50} \approx 10^{-15}$ (effectively zero). This 10 billion factor difference is why LSTMs work!
}}

\vspace{3mm}
\textbf{Visual Check:}

Can you explain to a friend how $C_t$ updates? If yes, you're ready for the full equations!
...
% CHECKPOINT 3: Test Equation Understanding
\begin{frame}[t]{Checkpoint 3: Can You Read LSTM Equations?}
\begin{center}
\textbf{Final Check: Do You Understand the Math?}
\end{center}
\vspace{3mm}
...
\column{0.48\textwidth}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 1:} In $f_t = \sigma(W_f [h_{t-1}, x_t] + b_f)$, what does $[h_{t-1}, x_t]$ mean?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 2:} Which gate uses tanh (not sigmoid)?
...
\vspace{3mm}
\colorbox{checkpointBlue!15}{\parbox{0.45\textwidth}{\small
\textbf{Question 3:} How many parameters does an LSTM learn per gate?

\vspace{2mm}
A) 1 (just the gate value)\\
B) 2 (weight and bias)\\
C) Many (weight matrix + bias vector)\\
D) Infinite (it's continuous)
}}

\column{0.48\textwidth}
\textbf{Answers \& Why They Matter:}

\vspace{3mm}
\colorbox{successGreen!10}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Answer 1: C (Concatenation)}
...
\vspace{3mm}
\colorbox{successGreen!10}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Answer 2: C (Candidate $\tilde{C}_t$)}

$\tilde{C}_t = \tanh(...)$ uses tanh because cell state can be negative or positive. All three GATES ($f_t, i_t, o_t$) use sigmoid because they need 0-1 range for ``how much''!
}}

\vspace{3mm}
\colorbox{successGreen!10}{\parbox{0.45\textwidth}{\footnotesize
\textbf{Answer 3: C (Many)}

Each gate has $W$ (weight matrix) + $b$ (bias vector). For hidden=256, input=100: $W_f$ is 256×356, $b_f$ is 256. That's 91,392 parameters per gate! Total LSTM: \~365k parameters.
}}

\vspace{3mm}
\textbf{Mastery Check:}

Can you write out all 6 equations from memory? If yes, you truly understand LSTMs!
