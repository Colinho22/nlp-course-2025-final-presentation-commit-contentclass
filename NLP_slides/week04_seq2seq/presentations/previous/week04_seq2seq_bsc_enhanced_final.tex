...
\newcommand{\intuition}[1]{\begin{tcolorbox}[colback=intuitionpurple,colframe=purple!50!black,title={\faIcon{brain} Build Your Intuition}]#1\end{tcolorbox}}
\newcommand{\tryit}[1]{\begin{tcolorbox}[colback=cyan!10!white,colframe=cyan!75!black,title={\faIcon{hand-point-right} Try It Yourself}]#1\end{tcolorbox}}
...
\begin{frame}[t]{Build Your Intuition: The Translation Problem}
    ...
    \end{itemize}
    \highlight{The word order completely changes between languages!}
\end{frame}
...
\begin{frame}[t]{The Length Mismatch: Real Data}
    ...
    \checkpoint{
        If padding doesn't work, what's the solution?\\
        Hint: How do human translators handle this?
    }
\end{frame}
...
\begin{frame}[t]{Evolution of Translation Approaches}
    ...
    \begin{block}{Key Insights}
        \begin{itemize}
            ...
            \item \textbf{2014}: \highlight{Seq2Seq breakthrough} - 15 BLEU improvement!
...
\begin{frame}[fragile,t]{The Brilliant Insight: Two-Stage Process}
    ...
    \checkpoint{
        Why do we need TWO networks instead of modifying one RNN?
    }
\end{frame}
...
\begin{frame}[fragile,t]{Building Intuition: The Encoder}
    \intuition{
        The encoder is like a \textbf{reader} that builds understanding:
        \begin{itemize}
    ...
    \tryit{
        Track how the hidden state changes:
        \begin{itemize}
...
\begin{frame}[fragile,t]{Building Intuition: The Decoder}
    \intuition{
        The decoder is like a \textbf{writer} that generates from understanding:
        \begin{itemize}
...
\begin{frame}[t]{Training Trick: Teacher Forcing}
    ...
    \textit{You CAN'T use teacher forcing during testing - you don't have the true translations!}
\end{frame}
...
\begin{frame}[fragile,t]{The Compression Problem}
    \intuition{
        ...
        Same problem with seq2seq: Longer input $\rightarrow$ More information loss
    }
    ...
    \checkpoint{
        Why can't we just use a bigger context vector?
    }
\end{frame}
...
\begin{frame}[t]{Information Theory Analysis}
    ...
    \begin{block}{Key Insights}
        \textbf{Information capacity calculation:}
        \begin{itemize}
            \item Each word $\approx$ 10 bits of information
            \item 256-dim vector $\approx$ 256 bits capacity
            \item 5 words = 50 bits $\rightarrow$ Fits! (20\% utilization)
            \item 20 words = 200 bits $\rightarrow$ Fits (78\% utilization)
            \item 100 words = 1000 bits $\rightarrow$ Overflow! (Need 4x capacity)
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}[t]{Where Information Gets Lost}
    ...
    \highlight{Performance drops 72\% for long sentences!}
\end{frame}
...
\begin{frame}[t]{How Humans Translate (The Key Insight)}
    ...
    \checkpoint{
        What if the decoder could do this too - look back at specific encoder states?
    }
\end{frame}
...
\begin{frame}[t]{The Attention Solution}
    ...
    \begin{itemize}
        ...
        \item \highlight{70\% focus on "cat"} $\leftarrow$ Makes sense!
...
for t in range(max_length):
    ...
    # Use context instead of fixed vector!
        ...
        \vspace{0.5em}
        \misconception{
            \textbf{Q:} "Does attention look at future words?"\\
            \textbf{A:} No! Only at encoder (source) states, never future target words.
        }
    \end{columns}
\end{frame}
...
\begin{frame}[t]{The Impact of Attention}
    \centering
    \vspace{-0.5em}
    \includegraphics[width=0.8\textwidth]{../figures/seq2seq_performance.pdf}
    \vspace{0.5em}
    
    \begin{block}{Key Insights}
        \textbf{Performance improvements (Bahdanau et al., 2015):}
        \begin{itemize}
            \item Short sentences (<10 words): +3\% BLEU
            \item Medium sentences (10-20): +22\% BLEU
            \item Long sentences (>20): \highlight{+192\% BLEU}
            \item Attention solves the bottleneck problem!
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}[t]{Complete Mathematical Formulation}
    ...
    \begin{align}
        ...
        h_t^{dec} &= \text{LSTM}^{dec}([E^{dec}(y_{t-1}); c_t], h_{t-1}^{dec}) & \text{(Decode)}\\
        P(y_t | y_{<t}, X) &= \softmax(W_o [h_t^{dec}; c_t] + b_o) & \text{(Output probs)}
    \end{align}
...
def beam_search(encoder_outputs, beam_size=3):
    ...
    for t in range(max_length):
        ...
        for sequence, score in beams:
            ...
            # Keep top k words
            top_words = top_k(probs, beam_size)
    ...
    return best_completed()
        ...
        \vspace{0.5em}
        \textbf{Example with beam\_size=2:}
        \small
        \begin{itemize}
...
\begin{frame}[t]{BLEU Score: Evaluating Translation Quality}
    ...
    \begin{itemize}
        \item Brevity penalty: $BP = e^{1-6/6} = 1.0$ (same length)
        \item Geometric mean: $\sqrt[4]{0.83 \times 0.60 \times 0.25 \times 0.01} = 0.22$
        \item Final BLEU: $1.0 \times 0.22 = 0.22$
    \end{itemize}
    
    \highlight{Interpretation: 0.22 means "Understandable but needs improvement"}
\end{frame}
...
\begin{frame}[fragile,t]{From Seq2Seq to Transformers}
    ...
    \vspace{0.5em}
    \highlight{Key Insight:} Everything you learned today is the foundation of modern LLMs!\\
...
\begin{frame}[t]{Week 4 Summary: Key Takeaways}
    ...
    \vspace{0.5em}
    \checkpoint{
        Quick check: Can you explain why we need TWO networks for translation?\\
