...
OUTPUT_DIR = Path("figures_with_logo_200px")  # Final output with 200px logo
LOGO_SIZE = 200     # 5x original size
PADDING = 10
...
for pdf_path in sorted(FIGURES_DIR.glob("*.pdf")):
    ...
    try:
        ...
        # Bottom-right position (extend beyond bottom edge)
        logo_x = page_width - LOGO_SIZE - PADDING
        logo_y = page_height - LOGO_SIZE + 40  # Push 40px beyond edge (partial logo visible)
