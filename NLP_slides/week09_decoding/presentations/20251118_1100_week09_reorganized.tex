...
% === NEW SLIDE 7: FULL EXPLORATION TREE ===
\begin{frame}[t]{Full Exploration: Exponential Explosion}
\vspace{-0.3cm}
\begin{center}
\includegraphics[width=0.75\textwidth]{figures/full_exploration_tree_graphviz.pdf}
\end{center}

\bottomnote{Visualization shows why exploring all paths is computationally infeasible}
\end{frame}

% === SLIDE 8: COMPUTATIONAL REALITY ===
\begin{frame}[t]{The Computational Reality}
\vspace{-0.3cm}
\begin{center}
\includegraphics[width=0.75\textwidth]{figures/extreme_computational_cost_bsc.pdf}
\end{center}
\vspace{2mm}
\textbf{The Challenge}: Need middle ground between 1 path and 10 billion paths

\bottomnote{From microseconds (greedy) to days (full search) - where is the sweet spot?}
\end{frame}

% === SLIDE 9: THE EXTREMES (NEW - SPLIT 1 of 2) ===
\begin{frame}[t]{The Extremes: Coverage Comparison}
\vspace{-0.5cm}
\begin{center}
\includegraphics[width=0.70\textwidth]{figures/extreme_coverage_comparison_bsc.pdf}
\end{center}
\vspace{1mm}
\textbf{Key Insight}: Greedy explores 0.01\%, Full search explores 100\% - both are impractical

\bottomnote{The extremes show why we need something in between}
\end{frame}
