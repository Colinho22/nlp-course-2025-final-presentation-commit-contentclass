...
def generate_task_method_decision_tree():
    ...
    ax.text(0.5, 0.95, 'START: What kind of task?', ha='center', fontsize=24, weight='bold',
           bbox=dict(boxstyle='round', facecolor=COLOR_ACCENT, edgecolor=COLOR_MAIN, linewidth=3))

    # First split: Deterministic vs Creative
    ax.text(0.25, 0.82, 'Need exact/factual?', ha='center', fontsize=24, weight='bold')
    ax.text(0.75, 0.82, 'Need creative/diverse?', ha='center', fontsize=24, weight='bold')

    # Left branch: Deterministic tasks
    ax.text(0.25, 0.68, 'Translation\nQ&A\nSummarization', ha='center', fontsize=24,
           bbox=dict(boxstyle='round', facecolor=COLOR_LIGHT))
    ax.text(0.25, 0.55, '→ BEAM SEARCH\n(width=3-5)', ha='center', fontsize=24, weight='bold',
           color='white', bbox=dict(boxstyle='round', facecolor=COLOR_GREEN, linewidth=3))
    ax.text(0.25, 0.42, 'Or Temp=0.3-0.5\nfor consistency', ha='center', fontsize=24, style='italic')

    # Right branch: Creative tasks
    ax.text(0.75, 0.68, 'Creative Writing\nDialogue\nStorytelling', ha='center', fontsize=24,
           bbox=dict(boxstyle='round', facecolor=COLOR_LIGHT))

    # Second split: With/without repetition risk
    ax.text(0.65, 0.52, 'Short responses?\n(low repetition risk)', ha='center', fontsize=24)
    ax.text(0.65, 0.40, '→ NUCLEUS\n(p=0.9-0.95)', ha='center', fontsize=24, weight='bold',
           color='white', bbox=dict(boxstyle='round', facecolor=COLOR_GREEN))

    ax.text(0.85, 0.52, 'Long generation?\n(repetition risk)', ha='center', fontsize=24)
    ax.text(0.85, 0.40, '→ CONTRASTIVE\n(α=0.5-0.7)', ha='center', fontsize=24, weight='bold',
           color='white', bbox=dict(boxstyle='round', facecolor=COLOR_GREEN))

    # Special case: Code generation
    ax.text(0.5, 0.25, 'Special: Code Generation', ha='center', fontsize=24, weight='bold')
    ax.text(0.5, 0.18, '→ Greedy or Beam (correctness critical)', ha='center', fontsize=24)
    ax.text(0.5, 0.12, '→ Then verify syntax/semantics', ha='center', fontsize=24, style='italic', color=COLOR_GRAY)
    ...
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.set_title('Choosing the Right Decoding Method', fontsize=24, weight='bold')
