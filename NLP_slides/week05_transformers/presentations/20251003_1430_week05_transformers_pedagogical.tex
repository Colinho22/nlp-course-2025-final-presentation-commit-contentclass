...
\bottomnote{The fundamental question that launched the transformer revolution}
\end{frame}
...
\bottomnote{All 5,120 GPU cores can work simultaneously - no waiting!}
\end{frame}
...
\bottomnote{The key challenge: How to encode position in parallel?}
\end{frame}

% Slide 11: The Key Question
\begin{frame}[t]{Position WITHOUT Sequential Processing?}
\begin{center}
...
\vspace{5mm}
\bottomnote{Like adding a timestamp or GPS coordinate to each word}
\end{frame}
...
% Slide 12: HUMAN INTROSPECTION (ENHANCED with BSc box)
...
\bottomnote{Like giving each word both ``what it means'' and ``where it is''}
\end{frame}
...
\vspace{5mm}
\begin{center}
\includegraphics[width=0.75\textwidth]{../figures/sr_08_vector_addition.pdf}
\end{center}

\bottomnote{The name makes sense: we're encoding position into the numbers}
\end{frame}
...
% Slide 19: Multi-Head Attention (ENHANCED with BSc box)
...
\bottomnote{Multiple perspectives, all processed simultaneously}
\end{frame}
...
\textbf{Timeline:}\\
Word 1: [-] (10ms)\\
Word 2: [-] (10ms) \textit{(parallel)}\\
Word 3: [-] (10ms) \textit{(parallel)}\\
\textbf{Total: 10ms}
\end{columns}
...
\bottomnote{This is why transformers revolutionized AI - unlocked the full power of GPUs}
\end{frame}

% Slide 21: EXPERIMENTAL VALIDATION (ENHANCED with realworld box)
...
\vspace{5mm}
\begin{center}
\includegraphics[width=0.85\textwidth]{../figures/sr_11_speed_vs_quality.pdf}
\end{center}

\bottomnote{This was the moment transformers revolutionized NLP}
\end{frame}
...
\bottomnote{These three innovations together enabled the AI revolution}
\end{frame}
...
\bottomnote{The transformer revolution shows how architecture innovation unlocks hardware potential}
\end{frame}

% Slide 25: 2024 Applications (ENHANCED with realworld box)
...
\vspace{5mm}
\begin{center}
\includegraphics[width=0.95\textwidth]{../figures/sr_12_modern_applications_timeline.pdf}
\end{center}

\bottomnote{Without 100x speedup, training GPT-3 would take 10 years}
\end{frame}
...
Now that training is fast, we can train HUGE models

\vspace{5mm}
\textbf{Lab:} Build a transformer from scratch, visualize attention patterns

\bottomnote{The speed breakthrough unlocked the scale that enabled modern AI}
\end{frame}
