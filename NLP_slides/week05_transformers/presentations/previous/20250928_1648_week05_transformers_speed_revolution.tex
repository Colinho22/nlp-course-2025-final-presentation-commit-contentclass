...
\secondary{\footnotesize Even with parallelization tricks, you're looking at months of waiting}
\end{frame}
...
\vspace{5mm}
\textbf{Visualization:}\\
\secondary{\small Imagine 5,120 workers at a factory}\\
\secondary{\small Only 49 working}\\
\secondary{\small 5,071 standing around waiting}
\end{columns}
...
\secondary{\footnotesize All 5,120 GPU cores can work simultaneously - no waiting!}
\end{frame}
...
\secondary{\footnotesize The research team was ecstatic - looked like the solution}
\end{frame}
...
\secondary{\footnotesize Real-world sentences are often 20-50+ words - this was unusable}
\end{frame}
...
\secondary{\footnotesize Without RNN, we lost the sequence information that tells us word order}
\end{frame}
...
\vspace{5mm}
\secondary{\footnotesize Like adding a timestamp or GPS coordinate to each word}
\end{frame}
...
\secondary{\footnotesize Like giving each word both ``what it means'' and ``where it is''}
\end{frame}
...
\secondary{\footnotesize The name makes sense: we're encoding position into the numbers}
\end{frame}
...
\vspace{3mm}
\secondary{\footnotesize Imagine sine wave at different frequencies:}
\begin{itemize}
...
\secondary{\footnotesize Think of it like a barcode - each position has its own unique pattern}
\end{frame}
...
\vspace{8mm}
\secondary{\footnotesize Notice: All three steps happen for ALL words simultaneously - no sequential processing!}
\end{frame}
...
\vspace{5mm}
\secondary{\footnotesize Interpretation: ``cat'' focuses most on itself, but also incorporates ``sat'' and ``the''}
\end{frame}
...
\secondary{\footnotesize The information retrieval analogy: Query searches through Keys to find relevant Values}
\end{frame}
...
\column{0.24\textwidth}
\textbf{Head 2}\\
\secondary{\footnotesize Semantics}
\begin{itemize}
\item bank → river
...
\column{0.24\textwidth}
\textbf{Head 3}\\
\secondary{\footnotesize Position}
\begin{itemize}
\item Adjacent words
\item Local context
...
\vspace{8mm}
\textbf{Timeline:}\\
\secondary{\footnotesize Word 1: [----] (100ms)}\\
\secondary{\footnotesize Word 2: [----] (100ms)}\\
\secondary{\footnotesize Word 3: [----] (100ms)}\\
\secondary{\footnotesize Total: 300ms}
...
\vspace{8mm}
\textbf{Timeline:}\\
\secondary{\footnotesize Word 1: [-] (10ms)}\\
\secondary{\footnotesize Word 2: [-] (10ms) } \textit{(parallel)}\\
\secondary{\footnotesize Word 3: [-] (10ms) } \textit{(parallel)}\\
\secondary{\footnotesize Total: 10ms}
\end{columns}
...
\secondary{\footnotesize This was the moment transformers revolutionized NLP}
\end{frame}
...
\vspace{5mm}
\secondary{\footnotesize Key insight: Matrix multiplication enables parallel computation of all word pairs simultaneously}
\end{frame}
...
\secondary{\footnotesize Without 100x speedup, training GPT-3 would take 10 years}
\end{frame}
...
\secondary{\footnotesize Now that training is fast, we can train HUGE models}
...
{\large Questions?}\\
\vspace{10mm}
\secondary{\footnotesize Next: Lab - Implementing Transformers From Scratch}
\end{center}
\end{frame}
