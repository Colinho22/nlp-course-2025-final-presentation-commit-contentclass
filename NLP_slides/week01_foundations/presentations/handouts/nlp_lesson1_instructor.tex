...
\begin{frame}{Instructor Notes}
\small
\begin{block}{Exercise Goals}
\begin{itemize}
\item Students discover n-gram concept naturally
\item Experience the challenges of language modeling
\item Build intuition before formal methods
\item Encourage creative thinking
\end{itemize}
\end{block}

\begin{block}{Expected Student Discoveries}
\begin{itemize}
\item Most will create bigram/trigram-like methods
\item Some might notice word categories
\item All will struggle with unseen combinations
\item Discussion reveals need for better methods
\end{itemize}
\end{block}

\begin{block}{Time Management}
\begin{itemize}
\item Total: 90 minutes
\item Counting: 15 min
\item Method Design: 20 min
\item Testing: 15 min
\item Discussion: 10 min
\item Presentations: 20 min
\item Wrap-up: 10 min
\end{itemize}
\end{block}
\end{frame}

\begin{frame}{Answer Key for Instructors}
\small
\begin{block}{Most Frequent Next Words in the Text}
\begin{tabular}{ll}
``the'' $\rightarrow$ & dragon (4), friendly (3), knight (3), green (2)\\
``friendly'' $\rightarrow$ & dragon (4)\\
``dragon'' $\rightarrow$ & loved (1), and (2), was (2), offered (1)\\
``green'' $\rightarrow$ & meadows (3)\\
``knight'' $\rightarrow$ & was (2), and (2), a (1)\\
\end{tabular}
\end{block}

\begin{block}{Sample Predictions (Based on Frequency)}
\begin{enumerate}
\item ``The friendly \textbf{dragon}'' (appears 4 times)
\item ``fly over the \textbf{green}'' or \textbf{land} (context dependent)
\item ``The knight was \textbf{afraid}'' or \textbf{no} (both appear)
\item ``They would often \textbf{fly}'' (from context)
\item ``Once upon a \textbf{time}'' (fixed phrase)
\end{enumerate}
\end{block}
\end{frame}
